...
export async function GET(request: NextRequest) {
  ...
  // DODANO (26.8.2026): rojstnodnevne cestitke. Poslje samo organizacijam, ki
  // so to vklopile, in strankam s privolitvijo za trzenje.
  try {
    const res = await birthdaysGET(request)
    results.birthdays = await res.json()
  } catch (e: any) {
    results.birthdays = { error: e.message }
  }
