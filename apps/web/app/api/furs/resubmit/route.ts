...
export async function POST(req: NextRequest) {
  ...
  let query = admin
    ...
    // POPRAVLJENO (prelet 263): pogoj `.not('furs_zoi','is',null)` je
    // IZKLJUCIL prav tiste racune, ki so prijave najbolj potrebni.
    //
    // Zvonec je stel racune BREZ EOR - tudi tiste brez ZOI - gumb "Poslji v
    // potrditev" pa je nato zajel SAMO tiste z ZOI. Klik torej ni naredil
    // nicesar, uporabnik pa ni vedel, zakaj.
    //
    // Primer 11.9.2026: racun za 16 EUR je zaradi neuspelega zahtevka ostal
    // brez ZOI. Zvonec ga je kazal, poslati pa ga ni bilo mogoce.
    //
    // Kadar ZOI ni, ga izracuna `confirmWithFurs` sam (glej spodaj) in racun
    // gre z uradno oznako naknadne prijave.
    .gte('paid_at', from)
  ...
  for (const inv of preview) {
    ...
    const data: FursInvoiceData = {
      ...
      /**
       * PRELET 263: `presetZoi` posljemo SAMO, ce ZOI obstaja.
       *
       * Ko obstaja, je to natanko tisti z natisnjenega racuna - FURS mora
       * dobiti istega, sicer gost svojega listka ne more preveriti.
       *
       * Ko ga ni (prijava je odpovedala, preden je bil izracunan), ga
       * `confirmWithFurs` izracuna zdaj. Nov ZOI se NE ujema z natisnjenim
       * listkom, ki ga nima - racun je pri FURS pravilno prijavljen, gostov
       * listek pa ga na preveriracun.fu.gov.si ne more potrditi.
       *
       * To je zavestna izbira: prijavljen promet je boljsi od neprijavljenega.
       *
       * DATUM IZDAJE ostane datum PRODAJE (`paid_at`), ne danasnji - prijava
       * je naknadna, dogodek pa se je zgodil takrat.
       */
      ...(inv.zoi ? { presetZoi: inv.zoi } : {}),
    ...
    try {
      ...
      if (res.success && res.eor) {
        ...
        /**
         * PRELET 263: shranimo tudi ZOI, kadar ga prej ni bilo.
         *
         * Brez tega bi racun imel EOR brez zascitne oznake. Ob naslednjem
         * klicu bi bil ZOI izracunan ZNOVA in bi bil drugacen - FURS bi imel
         * dva zapisa za isti racun z razlicnima oznakama.
         *
         * Ce je ZOI ze obstajal, ga NE prepisujemo: shranjeni je tisti z
         * natisnjenega listka in mora ostati nedotaknjen.
         */
        const { error: saveErr } = await admin.from('payments').update({
          furs_eor: res.eor,
          furs_sent_at: new Date().toISOString(),
          ...(inv.zoi ? {} : (res.zoi ? { furs_zoi: res.zoi } : {})),
        }).eq('id', inv.paymentId)
        if (saveErr) {
